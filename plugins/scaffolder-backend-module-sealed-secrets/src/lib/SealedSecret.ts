...
export default class SealedSecret {
  ...
  public sealSecret(): string {
    ...
    const rsaCipherLength = new Uint8Array([
      rsaCipherText.length / 256,
      rsaCipherText.length % 256,
    ]);
    const result = this.encrypt(sessionKey);
    const resultBuffer = new Uint8Array(
      rsaCipherLength.byteLength + rsaCipherText.byteLength + result.byteLength,
    );
    ...
    resultBuffer.set(
      ...
      rsaCipherLength.byteLength + rsaCipherText.byteLength,
    );
  ...
  private encrypt(sessionKey: Buffer): ArrayBuffer {
    ...
    const encrypted = Buffer.concat([
      cipher.update(this.secret, 'utf8'),
      cipher.final(),
    ]);
