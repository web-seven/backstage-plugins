...
import { MultiEntityObjectPickerProps } from './schema';
...
export { MultiEntityObjectPickerSchema } from './schema';

/**
 * The underlying component that is rendered in the form for the `MultiEntityObjectPicker`
 * field extension.
 *
 * @public
 */
export const MultiEntityObjectPicker = (
  props: MultiEntityObjectPickerProps,
) => {
  const {
    onChange,
      ...
      if (entities?.catalogEntities.length === 1) {
        ...
        initialEntitiesNames =
          formStateEntitiesNames && formStateEntitiesNames.length
            ...
            : initialEntitiesNames ||
              formData?.map(entity => entity.metadata.name) ||
              [];
